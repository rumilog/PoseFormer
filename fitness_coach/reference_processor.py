...
def load_reference(exercise_type, references_dir='references'):
    """
    Load a processed reference. If processed files don't exist, automatically process the raw video.
    ...
    if not poses_3d_path.exists():
        print(f"⚠️ Processed reference data not found for {exercise_type}")
        print(f"🔄 Processing reference video on first run...")
        
        # Find the reference video
        project_root = Path(__file__).parent.parent
        video_dir = project_root / 'demo' / 'video'
        
        # Look for reference video (common names)
        video_candidates = list(video_dir.glob('*.mp4'))
        if not video_candidates:
            raise FileNotFoundError(
                f"No reference video found in {video_dir}. "
                f"Please add a reference video for {exercise_type}."
            )
        
        # Use the first video found (or could match by name)
        ref_video = video_candidates[0]
        print(f"📹 Found reference video: {ref_video.name}")
        ...
        try:
            process_reference_video(
                ...
                exercise_type=exercise_type,
                output_dir=ref_dir,
                n_samples=50  # Reduced for faster processing
            )
            print(f"✅ Reference video processed successfully!")
        except Exception as e:
            raise RuntimeError(
                f"Failed to process reference video: {e}\n"
                f"Please ensure demo/vis.py and all dependencies are available."
            )
    ...
    # Load 3D poses (now guaranteed to exist)
    if not poses_3d_path.exists():
